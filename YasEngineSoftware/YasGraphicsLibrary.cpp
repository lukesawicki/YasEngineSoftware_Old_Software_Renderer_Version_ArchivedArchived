...
namespace YasGL
{
    ...
    void drawLine(const Vector2D<float>& point0, const Vector2D<float>& point1, PixelsTable& pixelsTable, const Vector4D<uint8_t>& drawingColor)
    {
        ...
        float originalPoint0X = point0.x;
        float originalPoint0Y = point0.y;

        float originalPoint1X = point1.x;
        float originalPoint1Y = point1.y;

        Vector2D<float> copyPoint0(point0.x, point0.y);
        Vector2D<float> copyPoint1(point1.x, point1.y);
