...
SurfaceWithMathBasedEffects::SurfaceWithMathBasedEffects(int x, int y, int width, int height,
...
SurfaceWithMathBasedEffects::SurfaceWithMathBasedEffects(Vector2D<int> position, int width, int height,
                                                         const Vector4D<Uint8>& defaultColor) {
...
void SurfaceWithMathBasedEffects::drawLine(const Vector2D<float>& point0, const Vector2D<float>& point1,
  ...
  if (abs(deltaX) != abs(deltaY)) {
    ...
    if (abs(deltaX) > abs(deltaY)) {
      ...
      if (deltaX < 0) {
        ...
        // NEGATIVE SLOPE)
        if (deltaY > 0) {
          // && (DELTAS CONDITION DX > DY) && (DELTA X < 0 CONDITION) -> IT MEANS OCTAN 3(NEGATIVE SLOPE, POINTS IN "WRONG ORDER")
          ...
          // POSITIVE SLOPE // (deltaY < 0) && (DELTAS CONDITION DX > DY) && (DELTA X < 0 CONDITION) -> IT MEANS OCTAN 4(POSITIVE SLOPE, POINTS IN "WRONG ORDER")
        ...
        if (deltaX != 0) {
          // POSITIVE SLOPE
          if (deltaY > 0) {
            // && (DELTAS CONDITION DX > DY) && (DELTA X > 0 CONDITION) -> IT MEANS OCTAN 0(POSITIVE SLOPE, POINTS IN "CORRECT ORDER")
            ...
            // NEGATIVE SLOPE // (deltaY < 0) && (DELTAS CONDITION DX > DY) && (DELTA X > 0 CONDITION) -> IT MEANS OCTAN 7(NEGATIVE SLOPE, POINTS IN "CORRECT ORDER")
            for (int i = originalPoint0X; i <= originalPoint1X; i++) {
              drawPoint(x0, y0, drawingColor);
              x0++;
              if ((2 * (cumulativeError + deltaY)) > -deltaX) {
      ...
      if (deltaY < 0) {
        // DELTA Y < 0 CONDITION (IT MEANS WRONG ORDER (BECAUSE IN HERE Y IS LEADING AXIES)
        ...
        // NEGATIVE SLOPE
        if (deltaX > 0) {
          // && (DELTAS CONDITION DX < DY) && (DELTA Y < 0 CONDITION) IT MEANS OCTAN 6(NEGATIVE SLOPE, POINTS IN "WRONG ORDER")
          deltaX = static_cast<int>(point0.x - point1.x);
          deltaY = static_cast<int>(point0.y - point1.y);
          ...
          // POSITIVE SLOPE  // deltaX < 0 && (DELTAS CONDITION DX < DY) && (DELTA Y < 0 CONDITION) IT MEANS OCTAN 5(POSITIVE SLOPE, POINTS IN "WRONG ORDER")
          deltaX = static_cast<int>(point0.x - point1.x);
          deltaY = static_cast<int>(point0.y - point1.y);
          for (int i = originalPoint0Y; i <= originalPoint1Y; i++) {
            drawPoint(x0, y0, drawingColor);
        ...
        if (deltaY != 0) {
          // POSITIVE SLOPE
          if (deltaX > 0) {
            // && (DELTAS CONDITION DX < DY) && (DELTA Y > 0 CONDITION) -> IT MEANS OCTAN 1(POSITIVE SLOPE, POINT IN "CORRECT ORDER")
            ...
            // NEGATIVE SLOPE // (deltaX < 0) && (DELTAS CONDITION DX < DY) && (DELTA Y > 0 CONDITION) -> IT MEANS OCTAN 2(NEGATIVE SLOPE POINTS IN "CORRECT ORDER")
            for (int i = originalPoint0Y; i <= originalPoint1Y; i++) {
              drawPoint(x0, y0, drawingColor);
              y0++;
              if ((2 * (cumulativeError + deltaX)) > -deltaY) {
          ...
          // deltaY == 0 It is straight line where y is constant. So draw simple line from x0 to x1
...
void SurfaceWithMathBasedEffects::drawNumbersAsGroupOfLines(Vector2D<float>* vertices, int maximumNumberOfVertices,
...
void SurfaceWithMathBasedEffects::copyPixelsInToPIxelTable(PixelsTable& pixelsTable) {
  ...
  for (int i = 0; i < viewPortSizes.y; i++) {
    for (int j = 0; j < viewPortSizes.x; j++) {
      pixelsTable.pixels[NUMBER_OF_COLORS * ((posY + i) * pixelsTable.windowDimensions.x + posX + j) + RED_POSITION] =
        pixels[NUMBER_OF_COLORS * (i * viewPortSizes.x + j) + RED_POSITION];
      pixelsTable.pixels[NUMBER_OF_COLORS * ((posY + i) * pixelsTable.windowDimensions.x + posX + j) + GREEN_POSITION] =
        pixels[NUMBER_OF_COLORS * (i * viewPortSizes.x + j) + GREEN_POSITION];
      pixelsTable.pixels[NUMBER_OF_COLORS * ((posY + i) * pixelsTable.windowDimensions.x + posX + j) + BLUE_POSITION] =
        pixels[NUMBER_OF_COLORS * (i * viewPortSizes.x + j) + BLUE_POSITION];
      pixelsTable.pixels[NUMBER_OF_COLORS * ((posY + i) * pixelsTable.windowDimensions.x + posX + j) + ALPHA_POSITION] =
        pixels[NUMBER_OF_COLORS * (i * viewPortSizes.x + j) + ALPHA_POSITION];
