  ...
  for (int i = 0; i < static_cast<int>(text.size()); i++) {
    for (int j = 0; j < NUMBER_OF_CHARACTERS; j++) {
      if (text.at(i) == charactersTable[j]) {
        ...
        drawNumbersAsGroupOfLines(fonts.at(j)->verticesBaseData->worldVertices,
                                  fonts.at(j)->verticesBaseData->numberOfVertices, color, false, pixelsTable);
      }
    }
  }
}
...
void ScreenWriter::prepareFontVertices() {
  ...
  fonts[36]->verticesBaseData->localVertices[0].x = -1;
