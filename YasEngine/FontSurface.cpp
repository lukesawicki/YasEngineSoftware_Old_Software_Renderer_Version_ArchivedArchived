...
void FontSurface::drawLine(const Vector2D<float>& point0, const Vector2D<float>& point1,
...
unsigned int FontSurface::calculateMaximumNumberOfElementsToProcess(const unsigned int& primaryMaximum,
                                                                    bool connectedLines) {
  int maximum = 0;
...
void FontSurface::drawNumbersAsGroupOfLines(Vector2D<float>* vertices, int maximumNumberOfVertices,
                                            const Vector4D<Uint8>& color, bool areLinesContinuos) {
  int step = 1;
  ...
  for (int i = 0; i < viewPortSizes.y; i++) {
    for (int j = 0; j < viewPortSizes.x; j++) {
      pixelsTable.pixels[NUMBER_OF_COLORS * ((posY + i) * pixelsTable.windowDimensions.x + posX + j) + RED_POSITION] =
        pixels[NUMBER_OF_COLORS * (i * viewPortSizes.x + j) + RED_POSITION]; // + RED_POSITION];
      pixelsTable.pixels[NUMBER_OF_COLORS * ((posY + i) * pixelsTable.windowDimensions.x + posX + j) + GREEN_POSITION] =
        pixels[NUMBER_OF_COLORS * (i * viewPortSizes.x + j) + GREEN_POSITION]; // + GREEN_POSITION];
      pixelsTable.pixels[NUMBER_OF_COLORS * ((posY + i) * pixelsTable.windowDimensions.x + posX + j) + BLUE_POSITION] =
        pixels[NUMBER_OF_COLORS * (i * viewPortSizes.x + j) + BLUE_POSITION]; // + BLUE_POSITION];
      pixelsTable.pixels[NUMBER_OF_COLORS * ((posY + i) * pixelsTable.windowDimensions.x + posX + j) + ALPHA_POSITION] =
        pixels[NUMBER_OF_COLORS * (i * viewPortSizes.x + j) + ALPHA_POSITION]; // + ALPHA_POSITION];
