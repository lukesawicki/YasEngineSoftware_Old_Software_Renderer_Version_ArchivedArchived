...
SurfaceWithMathBasedEffects::SurfaceWithMathBasedEffects(
  ...
  position_.x = x;
  position_.y = y;
  view_port_sizes_.x = width;
  view_port_sizes_.y = height;
  pixels_ = new Uint8[view_port_sizes_.x * view_port_sizes_.y * kNumberOfColors];
...
SurfaceWithMathBasedEffects::SurfaceWithMathBasedEffects(
  ...
  view_port_sizes_.x = width;
  view_port_sizes_.y = height;
  this->pixels_ = new Uint8[view_port_sizes_.x * view_port_sizes_.y * kNumberOfColors];
  clearColor(defaultColor);
}

SurfaceWithMathBasedEffects::~SurfaceWithMathBasedEffects() { delete[] pixels_; }

void SurfaceWithMathBasedEffects::clearColor(
    const Vector4D<Uint8>& drawingColor) {
  for (int y = 0; y < view_port_sizes_.y; y++) {
    for (int x = 0; x < view_port_sizes_.x; x++) {
      pixels_[kNumberOfColors * (y * view_port_sizes_.x + x) + kRedPosition] =
          drawingColor.x;
      pixels_[kNumberOfColors * (y * view_port_sizes_.x + x) + kGreenPosition] =
          drawingColor.y;
      pixels_[kNumberOfColors * (y * view_port_sizes_.x + x) + kBluePosition] =
          drawingColor.z;
      pixels_[kNumberOfColors * (y * view_port_sizes_.x + x) + kAlphaPosition] =
          drawingColor.w;
    }
  }
}

void SurfaceWithMathBasedEffects::drawPoint(
    int x, int y, const Vector4D<Uint8>& drawingColor) {
  cartesianPositionToWindow(x, y);
  if (x >= 0 && x < view_port_sizes_.x && y >= 0 && y < view_port_sizes_.y) {
    pixels_[kNumberOfColors * (y * view_port_sizes_.x + x) + kRedPosition] =
        drawingColor.x;
    pixels_[kNumberOfColors * (y * view_port_sizes_.x + x) + kGreenPosition] =
        drawingColor.y;
...
void SurfaceWithMathBasedEffects::drawLine(
    const Vector2D<float>& point0, const Vector2D<float>& point1,
    const Vector4D<Uint8>& drawingColor) {
...
void SurfaceWithMathBasedEffects::drawNumbersAsGroupOfLines(
...
void SurfaceWithMathBasedEffects::copyPixelsInToPIxelTable(
    PixelsTable& pixelsTable) {
  int posX = position_.x;
  int posY = position_.y;
  ...
  int viewportIndex = 0;
  for (int i = 0; i < view_port_sizes_.y; i++) {
    for (int j = 0; j < view_port_sizes_.x; j++) {
      pixelsTable
          .pixels[kNumberOfColors *
                      ((posY + i) * pixelsTable.windowDimensions.x + posX + j) +
                  kRedPosition] =
          pixels_[kNumberOfColors * (i * view_port_sizes_.x + j) + kRedPosition];
      pixelsTable
          .pixels[kNumberOfColors *
                      ((posY + i) * pixelsTable.windowDimensions.x + posX + j) +
                  kGreenPosition] =
          pixels_[kNumberOfColors * (i * view_port_sizes_.x + j) + kGreenPosition];
      pixelsTable
          .pixels[kNumberOfColors *
                      ((posY + i) * pixelsTable.windowDimensions.x + posX + j) +
                  kBluePosition] =
          pixels_[kNumberOfColors * (i * view_port_sizes_.x + j) + kBluePosition];
      pixelsTable
          .pixels[kNumberOfColors *
                      ((posY + i) * pixelsTable.windowDimensions.x + posX + j) +
                  kAlphaPosition] =
          pixels_[kNumberOfColors * (i * view_port_sizes_.x + j) + kAlphaPosition];
      viewportIndex = viewportIndex + 1;
    }
    startPoint = startPoint + view_port_sizes_.x;
  }
}
